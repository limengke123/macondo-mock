...
export const generateSingleData = function (schemaPath: fs.PathLike, schemaName: string): Promise<[Dictionary<any>, string]> {
    return Promise.resolve()
        .then(() => readJsonFile<Dictionary<Dictionary<Ischema>>>(schemaPath))
        .then(schema => {
            if (!schema[ENTRY]) {
